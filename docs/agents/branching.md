...
**If HEAD is not on a `custom-` branch, the user is asking for changes on that branch.** Stay on it: do not create or switch to a new branch, and skip section 3 entirely.

## 2. Start from the latest date-versioned branch
When working from a `custom-` branch, make sure it is the latest one. Date-versioned branches are named `custom-` followed by the date in `yyyy-MM-dd` (e.g. `custom-2026-09-04`).
...
## 4. Do not commit
After completing the task, do not make a commit (and do not push, tag, or open a PR) unless the user explicitly asks in that turn. Leave changes in the working tree.
