                ...
                if(rs.next()){
                    ...
                    try {
                        ...
                        while(rs2.next()){
                            ...
                            if(!Sequel.mengedittf("dapurbarang","kode_brng=?","stok=stok+?",2,new String[]{rs2.getString("jml_retur"),rs2.getString("kode_brng")})){
                                sukses = false;
                            }
                        }
                    } catch (Exception e) {
                    ...
                    if (sukses) {
                        jur.clear();
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Retur_Beli_Dapur from set_akun2"), rs.getDouble("total"), 0);
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Retur_Beli_Dapur from set_akun2"), 0, rs.getDouble("total"));
                        if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_retur_beli"),"U","BATAL TRANSAKSI RETUR BELI BARANG DAPUR"+", OLEH "+akses.getkode());
                    }
                    if(sukses==true){
