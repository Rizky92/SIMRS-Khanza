    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoFaktur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                    ...
                    for(i=0;i<jml;i++){
                        if(Valid.SetAngka(tbDokter.getValueAt(i,0).toString())>0){
                            if(Sequel.menyimpantf2("dapur_detail_hibah","?,?,?,?,?,?","Transaksi Hibah",6,new String[]{
                                ...
                                if(!Sequel.mengedittf("dapurbarang","kode_brng=?","stok=stok+?",2,new String[]{tbDokter.getValueAt(i,0).toString(),tbDokter.getValueAt(i,1).toString()})){
                                    sukses = false;
                                }
                            }else{
                                sukses=false;
                ...
                if(sukses==true){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Hibah_Dapur from set_akun2"), sbttl, 0);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Hibah_Dapur from set_akun2"), 0, sbttl);
                    if (sukses) sukses = jur.simpanJurnalSMC(NoFaktur.getText(),"U","HIBAH BARANG DAPUR, OLEH "+akses.getkode());
                }
