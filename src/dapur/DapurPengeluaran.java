    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoKeluar.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Stok_Keluar_Dapur from set_akun"), ttl, 0);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Stok_Keluar_Dapur from set_akun"), 0, ttl);
                    if (sukses) sukses = jur.simpanJurnalSMC(NoKeluar.getText(),"U","PENGGUNAAN BARANG DAPUR KERING DAN BASAH"+", OLEH "+akses.getkode());
                }
