    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoFaktur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(akunpembelian, (ttl + meterai), 0);
                    if(ppn>0){
                        if (sukses) sukses = jur.tampung(PPN_Masukan, ppn, 0);
                    }
                    if (sukses) sukses = jur.tampung(akunbayar, 0, (ttl + ppn + meterai));
                    if (sukses) sukses = jur.simpanJurnalSMC(NoFaktur.getText(),"U","PEMBELIAN BARANG DAPUR KERING & BASAH "+", OLEH "+akses.getkode());
                }
                if(sukses){
                    Sequel.Commit();
                }else{
