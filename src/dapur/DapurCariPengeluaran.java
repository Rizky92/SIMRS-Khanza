        ...
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 1).toString().trim().equals("")) {
            ...
            try {
                ...
                try {
                    ...
                    jur.bersihkan();;
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Stok_Keluar_Dapur from set_akun"), "PERSEDIAAN BARANG", 0, total);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Stok_Keluar_Dapur from set_akun"), "KAS DI TANGAN", total, 0);
