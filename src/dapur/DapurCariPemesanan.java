            ...
            try {
                ...
                try {
                    ...
                    if(rs.next()){
                        ...
                        if (sukses) {
                            jur.clear();
                            if (sukses) sukses = jur.tampung(Penerimaan_Dapur, 0, rs.getDouble("total"));
                            if (rs.getDouble("ppn") > 0) {
                                if (sukses) sukses = jur.tampung(PPN_Masukan, 0, rs.getDouble("ppn"));
                            }
                            if (sukses) sukses = jur.tampung(Kontra_Penerimaan_Dapur, rs.getDouble("tagihan"), 0);
                            if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_faktur"),"U","BATAL TRANSAKSI PENERIMAAN BARANG DAPUR"+", OLEH "+akses.getkode());
                        }
                        if(sukses==true){
