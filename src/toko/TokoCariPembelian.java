           ...
           try {
               ...
               try {
                  ...
                  if(rs.next()){
                      ...
                      jur.clear();
                      if (sukses) sukses = jur.tampung(akunpengadaan, "PEMBELIAN",0,rs.getDouble("total"));
                      if(rs.getDouble("ppn")>0){
                          if (sukses) sukses = jur.tampung(PPN_Masukan,"PPN Masukan Toko",0,rs.getDouble("ppn"));
                      }
                      if (sukses) sukses = jur.tampung(Sequel.cariIsi("select tokopembelian.kd_rek from tokopembelian where tokopembelian.no_faktur =?",rs.getString("no_faktur")),"KAS DI TANGAN",rs.getDouble("tagihan"),0);
                      if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_faktur"),"U","PEMBATALAN PENGADAAN BARANG TOKO"+", OLEH "+akses.getkode());
                      if(sukses==true){
