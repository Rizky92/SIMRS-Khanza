    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoRetur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses==true){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Retur_Beli_Toko,"RETUR PEMBELIAN",0,ttl);
                    if (sukses) sukses = jur.tampung(Kontra_Retur_Beli_Toko,"KONTRA RETUR PEMBELIAN",ttl,0);
                    if (sukses) sukses = jur.simpanJurnalSMC(NoRetur.getText(),"U","RETUR PEMBELIAN TOKO"+", OLEH "+akses.getkode());
                }
