...
public final class SatuSehatKirimSpecimenRadiologi extends javax.swing.JDialog {
    ...
    public SatuSehatKirimSpecimenRadiologi(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,new String[]{
                ...
                "Radiologi Code","Radiologi System","Radiologi Display","ID Service Request","Kode Pemeriksaan","ID Specimen","Accession Number"
            }){
    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataSatuSehatSpecimenRadiologi.html", "Data Specimen Radiologi Satu Sehat", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataSatuSehatSpecimenRadiologi.wps", "Data Specimen Radiologi Satu Sehat", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataSatuSehatSpecimenRadiologi.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataSatuSehatSpecimenRadiologi.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
        ...
        for(i=0;i<tbObat.getRowCount();i++){
            ...
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&tbObat.getValueAt(i,13).toString().equals("")){
                try {
                    ...
                    String acsn=tbObat.getValueAt(i,14).toString();
                    if (acsn.isBlank()) {
                    ...
                    try{
                        ...
                        json=api.kirimSmc(link+"/Specimen", HttpMethod.POST, requestEntity);
            ...
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,4).toString().equals(""))&&(!tbObat.getValueAt(i,13).toString().equals(""))){
                try {
                    ...
                    try{
                        ...
                        System.out.println("Request JSON : "+json);
                        requestEntity = new HttpEntity(json,headers);
                        json=api.kirimSmc(link+"/Specimen/"+tbObat.getValueAt(i,13).toString(), HttpMethod.PUT, requestEntity);
                        System.out.println("Result JSON : "+json);
                        tbObat.setValueAt(false,i,0);
                    } catch (HttpClientErrorException | HttpServerErrorException e) {
                        System.out.println("ERROR JSON : " + e.getResponseBodyAsString());
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        ...
                        rs.getString("sampel_display"),rs.getString("id_servicerequest"),rs.getString("kd_jenis_prw"),rs.getString("id_specimen"),accession.getNoACSN(rs.getString("noorder"),rs.getString("kd_jenis_prw"))
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            ...
            try {
                ...
                while(rs.next()){
                    tabMode.addRow(new Object[]{
                        false,rs.getString("no_rawat"),rs.getString("no_rkm_medis"),rs.getString("nm_pasien"),rs.getString("no_ktp"),rs.getString("noorder"),
                        rs.getString("tgl_sampel")+" "+rs.getString("jam_sampel"),rs.getString("nm_perawatan"),rs.getString("sampel_code"),rs.getString("sampel_system"),
                        rs.getString("sampel_display"),rs.getString("id_servicerequest"),rs.getString("kd_jenis_prw"),rs.getString("id_specimen"),accession.ambilAccession(rs.getString("noorder"),rs.getString("kd_jenis_prw"))
                    });
                }
            } catch (Exception e) {
                System.out.println("Notif : "+e);
            ...
            new SwingWorker<Void, Object[]>() {
                ...
                protected Void doInBackground() throws Exception {
                    try (PreparedStatement ps = koneksi.prepareStatement(
                        "select reg_periksa.no_rawat, reg_periksa.no_rkm_medis, pasien.nm_pasien, pasien.no_ktp, permintaan_radiologi.noorder, permintaan_radiologi.tgl_sampel, permintaan_radiologi.jam_sampel, " +
                        "jns_perawatan_radiologi.nm_perawatan, satu_sehat_mapping_radiologi.sampel_code, satu_sehat_mapping_radiologi.sampel_system, satu_sehat_mapping_radiologi.sampel_display, " +
                        ...
                        "satu_sehat_accession_radiologi_smc.no_acsn from reg_periksa inner join pasien on reg_periksa.no_rkm_medis = pasien.no_rkm_medis inner join permintaan_radiologi on " +
                        "permintaan_radiologi.no_rawat = reg_periksa.no_rawat inner join permintaan_pemeriksaan_radiologi on permintaan_pemeriksaan_radiologi.noorder = permintaan_radiologi.noorder " +
                        ...
                        "satu_sehat_accession_radiologi_smc.kd_jenis_prw inner join jns_perawatan_radiologi on jns_perawatan_radiologi.kd_jenis_prw = permintaan_pemeriksaan_radiologi.kd_jenis_prw " +
                        "inner join satu_sehat_mapping_radiologi on satu_sehat_mapping_radiologi.kd_jenis_prw = jns_perawatan_radiologi.kd_jenis_prw inner join satu_sehat_servicerequest_radiologi on " +
                        "satu_sehat_servicerequest_radiologi.noorder = permintaan_pemeriksaan_radiologi.noorder and satu_sehat_servicerequest_radiologi.kd_jenis_prw = permintaan_pemeriksaan_radiologi.kd_jenis_prw " +
                        "left join satu_sehat_specimen_radiologi on satu_sehat_servicerequest_radiologi.noorder = satu_sehat_specimen_radiologi.noorder and satu_sehat_servicerequest_radiologi.kd_jenis_prw = " +
                        "satu_sehat_specimen_radiologi.kd_jenis_prw where reg_periksa.tgl_registrasi between ? and ? " + (cari.isBlank() ? "" : "and (reg_periksa.no_rawat like ? or reg_periksa.no_rkm_medis like ? " +
                        "or pasien.nm_pasien like ? or pasien.no_ktp like ? or jns_perawatan_radiologi.nm_perawatan like ? or satu_sehat_mapping_radiologi.sampel_code like ? or permintaan_radiologi.noorder like ?)")
                    )) {
                        int p = 0;
                        ps.setString(++p, tgl1);
                        ps.setString(++p, tgl2);
                        ...
                        try (ResultSet rs = ps.executeQuery()) {
                            while (rs.next()) {
                                publish(new Object[] {
                                    false, rs.getString("no_rawat"), rs.getString("no_rkm_medis"), rs.getString("nm_pasien"), rs.getString("no_ktp"), rs.getString("noorder"), rs.getString("tgl_sampel") + " " + rs.getString("jam_sampel"),
                                    rs.getString("nm_perawatan"), rs.getString("sampel_code"), rs.getString("sampel_system"), rs.getString("sampel_display"), rs.getString("id_servicerequest"), rs.getString("kd_jenis_prw"),
                                    rs.getString("id_specimen"), accession.getNoACSN(rs.getString("no_acsn"), rs.getString("noorder"), rs.getString("kd_jenis_prw"))
                                });
                            }
                        }
                    }
                ...
                protected void done() {
                    ...
                    LCount.setText(tabMode.getRowCount() + "");
                    SatuSehatKirimSpecimenRadiologi.this.setCursor(Cursor.getDefaultCursor());
                    ceksukses = false;
                }
            }.execute();
        }
