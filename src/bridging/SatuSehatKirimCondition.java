    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataSatuSehatClinicalImpression.html", "Data Clinical Impression Satu Sehat", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataSatuSehatClinicalImpression.wps", "Data Clinical Impression Satu Sehat", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataSatuSehatClinicalImpression.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataSatuSehatClinicalImpression.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
            ...
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,5).toString().equals(""))&&(!tbObat.getValueAt(i,9).toString().equals(""))&&tbObat.getValueAt(i,12).toString().equals("")){
                try {
                    ...
                    try{
                        ...
                        json=api.kirimSmc(link+"/Condition", HttpMethod.POST, requestEntity);
            ...
            if(tbObat.getValueAt(i,0).toString().equals("true")&&(!tbObat.getValueAt(i,5).toString().equals(""))&&(!tbObat.getValueAt(i,9).toString().equals(""))&&(!tbObat.getValueAt(i,12).toString().equals(""))){
                try {
                    ...
                    try{
                        ...
                        System.out.println("Request JSON : "+json);
                        requestEntity = new HttpEntity(json,headers);
                        json=api.kirimSmc(link+"/Condition/"+tbObat.getValueAt(i,12).toString(), HttpMethod.PUT, requestEntity);
                        System.out.println("Result JSON : "+json);
                        tbObat.setValueAt(false,i,0);
                    } catch (HttpClientErrorException | HttpServerErrorException e) {
                        System.out.println("ERROR JSON : " + e.getResponseBodyAsString());
