    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataSatuSehatMedicationStatement.html", "Data Medication Statement Satu Sehat", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataSatuSehatMedicationStatement.wps", "Data Medication Statement Satu Sehat", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataSatuSehatMedicationStatement.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataSatuSehatMedicationStatement.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
                ...
                try {
                    ...
                    if(tbObat.getValueAt(i,27).toString().equals("")){
                        try{
                            ...
                            json=api.kirimSmc(link+"/MedicationStatement", HttpMethod.POST, requestEntity);
                ...
                try {
                    ...
                    if(tbObat.getValueAt(i,27).toString().equals("")){
                        try{
                            ...
                            System.out.println("Request JSON : "+json);
                            requestEntity = new HttpEntity(json,headers);
                            json=api.kirimSmc(link+"/MedicationStatement/"+tbObat.getValueAt(i,26).toString(), HttpMethod.PUT, requestEntity);
                            System.out.println("Result JSON : "+json);
                            tbObat.setValueAt(false,i,0);
                        } catch (HttpClientErrorException | HttpServerErrorException e) {
                            System.out.println("ERROR JSON : " + e.getResponseBodyAsString());
