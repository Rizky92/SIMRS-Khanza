...
public class DlgUpdateUser extends javax.swing.JDialog {
    ...
    private boolean penyakit=false,obat_penyakit=false,dokter=false,jadwal_praktek=false,petugas=false,pasien=false,registrasi=false,tindakan_ralan=false,kamar_inap=false,
            ...
            skrining_indra_pendengaran=false,catatan_pengkajian_paska_operasi=false,skrining_frailty_syndrome=false,sirkulasi_cssd=false,gabung_kamar_inap=false,lama_pelayanan_cssd=false,catatan_observasi_bayi=false,
    ...
    */
    ...
    private void tampil(String user) {
        ...
        try{
            ...
            ps=koneksi.prepareStatement(
                ...
                "user.skrining_risiko_kanker_serviks,user.catatan_cairan_hemodialisa,user.skrining_kesehatan_gigi_mulut_lansia,user.skrining_indra_pendengaran,user.catatan_pengkajian_paska_operasi,gabung_kamar_inap,"+
    ...
    private void setTampilSmc() {
        if ("[A]Gabung Kamar Inap".toLowerCase().contains(TCari.getText().toLowerCase())) {
            tabMode.addRow(new Object[] {false, "[A]Gabung Kamar Inap", gabung_kamar_inap});
        }
    }

    private void simpansmc() {
        if ("[A]Gabung Kamar Inap".equals(tbUser.getValueAt(i, 1).toString())) {
            Sequel.mengupdateSmc("user", "gabung_kamar_inap = ?", "id_user = aes_encrypt(?, 'nur')", tbUser.getValueAt(i, 2).toString(), TKd.getText());
        }
    }
}