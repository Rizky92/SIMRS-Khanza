    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("PerbaikanInventaris.html", "Data Perbaikan Inventaris", tbJnsPerawatan);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("PerbaikanInventaris.wps", "Data Perbaikan Inventaris", tbJnsPerawatan);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("PerbaikanInventaris.csv", tbJnsPerawatan);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("PerbaikanInventaris.xlsx", tbJnsPerawatan);
                        break;
                    case "Laporan 5 (Jasper)":
                        ...
                        Valid.reportSmc("rptPerbaikanInventaris.jasper", "report", "::[ Data Perbaikan Inventaris ]::", param, "select perbaikan_inventaris.no_permintaan, " +
                            "permintaan_perbaikan_inventaris.no_inventaris, inventaris.kode_barang, inventaris_barang.nama_barang, inventaris_ruang.nama_ruang, " +
                            "perbaikan_inventaris.nip, petugas.nama, perbaikan_inventaris.uraian_kegiatan, perbaikan_inventaris.tanggal, perbaikan_inventaris.pelaksana, " +
                            "perbaikan_inventaris.biaya, perbaikan_inventaris.keterangan, perbaikan_inventaris.status from perbaikan_inventaris inner join " +
                            "permintaan_perbaikan_inventaris on perbaikan_inventaris.no_permintaan = permintaan_perbaikan_inventaris.no_permintaan inner join " +
                            "inventaris on permintaan_perbaikan_inventaris.no_inventaris = inventaris.no_inventaris inner join inventaris_barang on " +
                            "inventaris.kode_barang = inventaris_barang.kode_barang inner join inventaris_ruang on inventaris.id_ruang = inventaris_ruang.id_ruang " +
                            "inner join petugas on perbaikan_inventaris.nip = petugas.nip where perbaikan_inventaris.tanggal between ? and ? and (if(trim(?) = '', " +
                            "1 = 1, inventaris_ruang.nama_ruang like ?)) and (if(trim(?) = '', 1 = 1, perbaikan_inventaris.no_permintaan like ? or " +
                            "permintaan_perbaikan_inventaris.no_inventaris like ? or inventaris.kode_barang like ? or inventaris_barang.nama_barang like ? " +
                            "or perbaikan_inventaris.pelaksana like ? or perbaikan_inventaris.status like ? or petugas.nama like ?)) order by perbaikan_inventaris.no_permintaan",
                            Valid.getTglSmc(DTPCari1), Valid.getTglSmc(DTPCari2), nm_ruangcari.getText().trim(), "%" + nm_ruangcari.getText().trim() + "%", TCari.getText().trim(),
