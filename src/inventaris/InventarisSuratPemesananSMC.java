...
public class InventarisSuratPemesananSMC extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        internalFrame5.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(230, 235, 225)), "::[ Cetak Surat Pemesanan Barang Aset/Inventaris ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 70, 50))); // NOI18N
        ...
        internalFrame1.setBorder(javax.swing.BorderFactory.createTitledBorder(javax.swing.BorderFactory.createLineBorder(new java.awt.Color(240, 245, 235)), "::[ Surat Pemesanan Barang Aset/Inventaris ]::", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 11), new java.awt.Color(50, 50, 50))); // NOI18N
        ...
        jLabel11.setText("Akun Aset :");
        panelisi3.add(jLabel11);
        jLabel11.setBounds(385, 40, 75, 23);
        ...
        AkunAset.setBounds(463, 40, 299, 23);
    ...
    private void kdsupKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdsupKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
            Sequel.cariIsi("select inventaris_suplier.nama_suplier from inventaris_suplier where inventaris_suplier.kode_suplier=?", nmsup, kdsup.getText());
    ...
    private void formWindowOpened(java.awt.event.WindowEvent evt) {//GEN-FIRST:event_formWindowOpened
        if (Valid.umurcacheSmc("./cache/akunaset.iyem", 7)) {
            tampilAkun();
        } else {
            tampilAkun2();
        }
    ...
    private void BtnCariActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCariActionPerformed
        ...
        InventarisCariSuratPemesananSMC form = new InventarisCariSuratPemesananSMC(null, false);
        form.emptTeks();
        form.isCek();
    ...
    private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
        if (tbDokter.getRowCount() != 0) {
            if (evt.getKeyCode() == KeyEvent.VK_ENTER) {
                ...
                if ((tbDokter.getSelectedColumn() == 1) || (tbDokter.getSelectedColumn() == 7) || (tbDokter.getSelectedColumn() == 10)) {
                    getData();
                } else if (tbDokter.getSelectedColumn() == 9) {
                    try {
                        tbDokter.setValueAt(Math.round(Double.parseDouble(tbDokter.getValueAt(tbDokter.getSelectedRow(), 8).toString()) *
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (NoPemesanan.getText().trim().equals("")) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                sukses = true;
                if (Sequel.menyimpantfSmc("surat_pemesanan_inventaris_smc", "", NoPemesanan.getText(), kdsup.getText(), kdptg.getText(),
                    Sequel.cariIsiSmc("select rekening.kd_rek from rekening where rekening.nm_rek = ?", AkunAset.getSelectedItem().toString()),
                    Valid.getTglSmc(Tanggal), "" + sbttl, "" + ttldisk, "" + ttl,"" + ppn, "" + meterai, "" + (ttl + ppn + meterai), "Proses Pesan"
                )) {
                    jml = tbDokter.getRowCount();
                    for (int i = 0; i < jml; i++) {
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (!Sequel.menyimpantfSmc("detail_surat_pemesanan_inventaris_smc", "", NoPemesanan.getText(),
                                tbDokter.getValueAt(i, 1).toString(), tbDokter.getValueAt(i, 0).toString(), tbDokter.getValueAt(i, 7).toString(),
    ...
    private void BtnPrint5ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrint5ActionPerformed
        ...
        if (NoPemesanan.getText().trim().equals("")) {
            ...
            for (int i = 0; i < row; i++) {
                if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                    Sequel.menyimpan("temporary", "'" + i + "','" +
                        ...
                        Valid.SetAngka(Double.parseDouble(tabMode.getValueAt(i, 11).toString())) + "','','','','','','','','','','','','','','','','','','','','','','','','','','','','" + akses.getalamatip() + "'",
                        "Transaksi Pemesanan");
                }
            }
            ...
            Valid.MyReportqry("rptSuratPemesananInventarisSmc.jasper", "report", "::[ Transaksi Pemesanan Barang Aset ]::", "select * from temporary where temporary.temp37='" + akses.getalamatip() + "' order by temporary.no", param);
        }
        this.setCursor(Cursor.getDefaultCursor());
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(() -> {
            InventarisSuratPemesananSMC dialog = new InventarisSuratPemesananSMC(new javax.swing.JFrame(), true);
        ...
        if (!ceksukses) {
            ...
            try {
                ...
                new SwingWorker<Void, Object[]>() {
                    ...
                    final String akun = AkunAset.getSelectedItem().toString();
                    ...
                    protected Void doInBackground() throws Exception {
                        try (FileWriter fw = new FileWriter(file); ResultSet rs = koneksi.createStatement().executeQuery(
                            ...
                            "ifnull(j.nama_jenis, '') as nama_jenis, ifnull(r.nm_rek, '') as nama_rek from inventaris_barang b left join inventaris_produsen p on b.kode_produsen = p.kode_produsen " +
                            "left join inventaris_merk m on b.id_merk = m.id_merk left join inventaris_kategori k on b.id_kategori = k.id_kategori left join inventaris_jenis j on b.id_jenis = j.id_jenis " +
                            "left join akun_aset_inventaris a on b.id_jenis = a.id_jenis left join rekening r on a.kd_rek = r.kd_rek order by b.nama_barang"
                        )) {
                            ...
                            while (rs.next()) {
                                ...
                                node.put("Akun", rs.getString(7));
                                array.add(node);
                                if (rs.getString(7).equals(akun)) {
                                    if (cari.isBlank()) {
                                        publish(new Object[] {
                                        ...
                                        if (rs.getString(1).toLowerCase().contains(cari)
                                            ...
                                            || rs.getString(5).toLowerCase().contains(cari)
                                            || rs.getString(6).toLowerCase().contains(cari)
                                        ) {
                                            publish(new Object[] {
                                                "", rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), 0, 0, 0, 0, 0
                                            });
                    ...
                    try (FileReader fr = new FileReader(new File("./cache/suratpemesananinventarissmc.iyem"))) {
                        ...
                        if (cari.isBlank()) {
                            for (JsonNode node : array) {
                                if (node.path("Akun").asText("").equals(akun)) {
    ...
    private void tampilAkun() {
        try {
            File file = new File("./cache/akunaset.iyem");
            file.createNewFile();
            try (FileWriter fw = new FileWriter(file); ResultSet rs = koneksi.createStatement().executeQuery(
                "select rekening.nm_rek, akun_aset_inventaris.kd_rek from akun_aset_inventaris inner join rekening on " +
                "akun_aset_inventaris.kd_rek = rekening.kd_rek group by rekening.nm_rek order by rekening.nm_rek"
            )) {
                ...
                if (rs.next()) {
                    do {
                        ObjectNode node = mapper.createObjectNode();
                        node.put("NamaAkun", rs.getString(1));
                        node.put("KodeRek", rs.getString(2));
                        array.add(node);
                        AkunAset.addItem(rs.getString(1));
                    } while (rs.next());
                    fw.write(mapper.writeValueAsString(mapper.createObjectNode().set("akunaset", array)));
                    fw.flush();
                } else {
                    JOptionPane.showMessageDialog(null, "Tidak dapat menemukan akun per jenis aset\nSilahkan cek pengaturan akun aset/inventaris..!!", "Peringatan", JOptionPane.WARNING_MESSAGE);
                }
            }
        } catch (Exception e) {
            System.out.println("Notif : " + e);
    ...
    private void tampilAkun2() {
        try {
            if (new File("./cache/akunaset.iyem").isFile() && !Valid.umurcacheSmc("./cache/akunaset.iyem", 30)) {
    ...
    public void isCek() {
        ...
        if (akses.getjml2() >= 1) {
            ...
            BtnSimpan.setEnabled(akses.getsurat_pemesanan_inventaris_smc());
