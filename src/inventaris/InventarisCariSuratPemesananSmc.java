...
public class InventarisCariSuratPemesananSmc extends javax.swing.JDialog {
    ...
    public InventarisCariSuratPemesananSmc(java.awt.Frame parent, boolean modal) {
        ...
        int[] widths = {90, 300, 40, 90, 90, 50, 90, 95};
    ...
    private void ppDatangActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppDatangActionPerformed
        if (tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim().equals("")) {
            ...
            if (Sequel.cariIsi("select status from surat_pemesanan_inventaris_smc where no_pemesanan=?", tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()).equals("Sudah Datang")) {
                JOptionPane.showMessageDialog(null, "Data pemesanan sudah tervalidasi..!!");
            } else {
                Sequel.queryu("update surat_pemesanan_inventaris_smc set status='Sudah Datang' where no_pemesanan=?", tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString().trim());
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                ...
                " inner join inventaris_suplier is2 on spi.kode_suplier = is2.kode_suplier" +
                " inner join petugas p on spi.nip = p.nip" +
                " inner join detail_surat_pemesanan_inventaris_smc dspi on spi.no_pemesanan = dspi.no_pemesanan" +
                " inner join inventaris_barang ib on dspi.kode_barang = ib.kode_barang" +
                " where spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and spi.no_pemesanan like ? or" +
                " spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and spi.kode_suplier like ? or" +
                " spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and is2.nama_suplier like ? or" +
                " spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and spi.nip like ? or" +
                " spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and p.nama like ? or" +
                " spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and dspi.kode_barang like ? or" +
                " spi.tanggal between ? and ? and spi.no_pemesanan like ? and is2.nama_suplier like ? and p.nama like ? and ib.nama_barang like ? and ib.nama_barang like ?" +
                " group by spi.no_pemesanan order by spi.tanggal, spi.no_pemesanan");
            try {
                String tgl1 = Valid.SetTgl(TglBeli1.getSelectedItem() + "");
                String tgl2 = Valid.SetTgl(TglBeli2.getSelectedItem() + "");
                String noFak = "%" + NoFaktur.getText() + "%";
                String sup = "%" + nmsup.getText() + "%";
                String ptg = "%" + nmptg.getText() + "%";
                String bar = "%" + nmbar.getText() + "%";
                String q = "%" + TCari.getText() + "%";
                // 7 OR branches × 7 params = 49
                for (int b = 0; b < 7; b++) {
                    int base = b * 7;
                    ps.setString(base + 1, tgl1);
                    ps.setString(base + 2, tgl2);
                    ps.setString(base + 3, noFak);
                    ps.setString(base + 4, sup);
                    ps.setString(base + 5, ptg);
                    ps.setString(base + 6, bar);
                    ps.setString(base + 7, q);
                }
                rs = ps.executeQuery();
                tagihan = 0;
                while (rs.next()) {
                    ...
                    ps2 = koneksi.prepareStatement(
                        ...
                        " from detail_surat_pemesanan_inventaris_smc dspi" +
                        " inner join inventaris_barang ib on dspi.kode_barang = ib.kode_barang" +
                        " where (dspi.no_pemesanan=? and ib.nama_barang like ? and dspi.kode_barang like ?)" +
                        " or (dspi.no_pemesanan=? and ib.nama_barang like ? and ib.nama_barang like ?)" +
                        " order by dspi.kode_barang");
                    try {
                        String noPem = rs.getString("no_pemesanan");
                        ps2.setString(1, noPem);
                        ps2.setString(2, bar);
                        ps2.setString(3, q);
                        ps2.setString(4, noPem);
                        ps2.setString(5, bar);
                        ps2.setString(6, bar);
                        rs2 = ps2.executeQuery();
                        int no = 1;
                        while (rs2.next()) {
                            tabMode.addRow(new Object[]{"",
                                no + ". " + rs2.getString("kode_barang") + ", " + rs2.getString("nama_barang"),
                                rs2.getString("jumlah"), Valid.SetAngka(rs2.getDouble("h_pesan")),
                                Valid.SetAngka(rs2.getDouble("subtotal")), Valid.SetAngka(rs2.getDouble("dis")),
