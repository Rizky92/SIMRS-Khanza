        ...
        if(NoFaktur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses){
                    ...
                    if (!Sequel.insertTampJurnal(akunaset, "PEMBELIAN", (ttl + meterai), 0)) {
                        sukses = false;
                    }
                    if(ppn>0){
                        if (!Sequel.insertTampJurnal(PPN_Masukan, "PPN Masukan Inventaris", ppn, 0)) {
                            sukses = false;
                        }
                    }
                    if (!Sequel.insertTampJurnal(akunbayar, "KAS KELUAR", 0, (ttl + ppn + meterai))) {
                        sukses = false;
                    }
                    if (sukses) {
