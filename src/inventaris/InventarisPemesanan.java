    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoFaktur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses){
                    jur.bersihkan();
                    if (sukses) sukses = jur.tampung(akunaset, "PENERIMAAN ASET INVENTARIS", (ttl + meterai), 0);
                    if(ppn>0){
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select PPN_Masukan from set_akun"), "PPN Masukan Barang Inventaris", ppn, 0);
                    }
