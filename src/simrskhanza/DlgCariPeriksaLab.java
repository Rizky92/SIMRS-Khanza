    ...
    public void HasilLabPDFKlaimBPJS(String NomorRawat, String NoRekamMedis)
    {
        ...
        try {
            ...
            try {
                ...
                rs = ps4.executeQuery();
                while (rs.next()) {
                    kamar = Sequel.cariIsiSmc("select ifnull(kd_kamar, '') from kamar_inap where no_rawat = ? order by concat(tgl_masuk, ' ', jam_masuk) desc limit 1", rs.getString("no_rawat"));
                    if (!kamar.isBlank()) {
                        namakamar = kamar + ", " + Sequel.cariIsiSmc("select nm_bangsal from bangsal join kamar on bangsal.kd_kamar = kamar.kd_bangsal where kamar.kd_kamar = ?", kamar);
                        kamar = "Kamar";
                    } else {
                        kamar = "Poli";
                        namakamar = Sequel.cariIsiSmc("select nm_poli from poliklinik join reg_periksa on poliklinik.kd_poli = reg_periksa.kd_poli where reg_periksa.no_rawat = ?", rs.getString("no_rawat"));
                    }
                    
                    Map<String, Object> param = new HashMap<>();
                    param.put("noperiksa", rs.getString("no_rawat"));
                    param.put("norm", rs.getString("no_rkm_medis") + " | Tgl. Lahir : " + rs.getString("lahir"));
                    param.put("namapasien", rs.getString("nm_pasien"));
                    param.put("jkel", rs.getString("jk"));
                    param.put("umur", rs.getString("umur"));
