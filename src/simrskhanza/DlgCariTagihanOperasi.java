                ...
                if (reply == JOptionPane.YES_OPTION) {
                    ...
                    if(sukses==true){
                        ...
                        if(status.equals("Ranap")){
                            ...
                            if(ttlpendapatan>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(Suspen_Piutang_Operasi_Ranap, "Suspen Piutang Operasi Ranap", 0, ttlpendapatan);
                                if (sukses) sukses = Sequel.insertTampJurnal(Operasi_Ranap, "Pendapatan Operasi Rawat Inap", ttlpendapatan, 0);
                            }
                            if(ttljmdokter>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Dokter_Operasi_Ranap, "Beban Jasa Medik Dokter Operasi Ranap", 0, ttljmdokter);
                                if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Dokter_Operasi_Ranap, "Utang Jasa Medik Dokter Operasi Ranap", ttljmdokter, 0);
                            }
                            if(ttljmpetugas>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Paramedis_Operasi_Ranap, "Beban Jasa Medik Petugas Operasi Ranap", 0, ttljmpetugas);
                                if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Paramedis_Operasi_Ranap, "Utang Jasa Medik Petugas Operasi Ranap", ttljmpetugas, 0);
                            }
                            if(ttlbhp>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(HPP_Obat_Operasi_Ranap, "HPP Persediaan Operasi Rawat Inap", 0, ttlbhp);
                                if (sukses) sukses = Sequel.insertTampJurnal(Persediaan_Obat_Kamar_Operasi_Ranap, "Persediaan BHP Operasi Rawat Inap", ttlbhp, 0);
                            }
                            ...
                            Sequel.deleteTampJurnal();    
                            if(ttlpendapatan>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(Suspen_Piutang_Operasi_Ralan, "Suspen Piutang Operasi Ralan", 0, ttlpendapatan);
                                if (sukses) sukses = Sequel.insertTampJurnal(Operasi_Ralan, "Pendapatan Operasi Rawat Jalan", ttlpendapatan, 0);
                            }
                            if(ttljmdokter>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Dokter_Operasi_Ralan, "Beban Jasa Medik Dokter Operasi Ralan", 0, ttljmdokter);
                                if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Dokter_Operasi_Ralan, "Utang Jasa Medik Dokter Operasi Ralan", ttljmdokter, 0);
                            }
                            if(ttljmpetugas>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Paramedis_Operasi_Ralan, "Beban Jasa Medik Petugas Operasi Ralan", 0, ttljmpetugas);
                                if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Paramedis_Operasi_Ralan, "Utang Jasa Medik Petugas Operasi Ralan", ttljmpetugas, 0);
                            }
                            if(ttlbhp>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(HPP_Obat_Operasi_Ralan, "HPP Persediaan Operasi Rawat Jalan", 0, ttlbhp);
                                if (sukses) sukses = Sequel.insertTampJurnal(Persediaan_Obat_Kamar_Operasi_Ralan, "Persediaan BHP Operasi Rawat Jalan", ttlbhp, 0);
                            }
                    ...
                    if(Sequel.queryutf("delete from beri_obat_operasi where no_rawat='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),1) +"' and tanggal='"+tbDokter.getValueAt(tbDokter.getSelectedRow(),0)+"'")==true){
                        if(status.equals("Ranap")){
                            if(ttlbhp>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(HPP_Obat_Operasi_Ranap, "HPP Persediaan Operasi Rawat Inap", 0, ttlbhp);
                                if (sukses) sukses = Sequel.insertTampJurnal(Persediaan_Obat_Kamar_Operasi_Ranap, "Persediaan BHP Operasi Rawat Inap", ttlbhp, 0);
                                if (sukses) sukses = Sequel.insertTampJurnal(Suspen_Piutang_Operasi_Ranap, "Suspen Piutang Operasi Ranap", 0, ttlbhp);
                                if (sukses) sukses = Sequel.insertTampJurnal(Operasi_Ranap, "Pendapatan Operasi Rawat Inap", ttlbhp, 0);
                            ...
                            if(ttlbhp>0){
                                if (sukses) sukses = Sequel.insertTampJurnal(HPP_Obat_Operasi_Ralan, "HPP Persediaan Operasi Rawat Jalan", 0, ttlbhp);
                                if (sukses) sukses = Sequel.insertTampJurnal(Persediaan_Obat_Kamar_Operasi_Ralan, "Persediaan BHP Operasi Rawat Jalan", ttlbhp, 0);
                                if (sukses) sukses = Sequel.insertTampJurnal(Suspen_Piutang_Operasi_Ralan, "Suspen Piutang Operasi Ralan", 0, ttlbhp);
                                if (sukses) sukses = Sequel.insertTampJurnal(Operasi_Ralan, "Pendapatan Operasi Rawat Jalan", ttlbhp, 0);
