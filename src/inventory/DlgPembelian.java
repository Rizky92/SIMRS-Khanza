            ...
            if (reply == JOptionPane.YES_OPTION) {
                try {
                    ...
                    if(Sequel.menyimpantf2("pembelian","?,?,?,?,?,?,?,?,?,?,?","No.Faktur",11,new String[]{
                        ...
                        if(sukses==true){
                            jur.clear();
                            if (sukses) sukses = jur.tampung(Pengadaan_Obat, "PEMBELIAN", ttl, 0);
                            if(ppn>0){
                                if (sukses) sukses = jur.tampung(PPN_Masukan, "PPN Masukan Obat", ppn, 0);
                            }
                            if (sukses) sukses = jur.tampung(kode_akun_bayar, AkunBayar.getSelectedItem().toString(), 0, (ttl + ppn));
                            if (sukses) sukses = jur.simpanJurnalSMC(NoFaktur.getText(),"U","PEMBELIAN DI "+nmgudang.getText().toUpperCase()+", OLEH "+akses.getkode());
                        }
                    }else{
                        sukses=false;
