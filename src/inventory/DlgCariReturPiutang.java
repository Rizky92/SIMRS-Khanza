         ...
         try {
            ...
            if(rs.next()){
                ...
                if(sukses==true){
                    ...
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Retur_Piutang_Obat from set_akun"), "RETUR PIUTANG", 0, totalDetilReturPiutang);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Retur_Piutang_Obat from set_akun"), "KAS DI TANGAN", totalDetilReturPiutang, 0);
                    if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_retur_piutang"),"U","BATAL RETUR PENJUALAN DI "+Sequel.cariIsi("select bangsal.nm_bangsal from bangsal where bangsal.kd_bangsal='"+rs.getString("kd_bangsal")+"'").toUpperCase()+", OLEH "+akses.getkode());
                }
