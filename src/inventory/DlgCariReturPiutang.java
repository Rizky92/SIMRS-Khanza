         ...
         try {
            ...
            if(rs.next()){
                ...
                if(sukses==true){
                    ...
                    Sequel.insertTampJurnal(Sequel.cariIsi("select Retur_Piutang_Obat from set_akun"), "RETUR PIUTANG", 0, Sequel.cariDoubleSmc("select sum(subtotal) from detreturpiutang where no_retur_piutang = ?", rs.getString("no_retur_piutang")));
                    Sequel.insertTampJurnal(Sequel.cariIsi("select Kontra_Retur_Piutang_Obat from set_akun"), "KAS DI TANGAN", Sequel.cariDoubleSmc("select sum(subtotal) from detreturpiutang where no_retur_piutang = ?", rs.getString("no_retur_piutang")), 0);
