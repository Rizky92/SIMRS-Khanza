    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        if(NoNota.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses==true){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Piutang_Obat from set_akun"), "PIUTANG OBAT", (ttljual + ongkir - uangmuka), 0);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Piutang_Obat from set_akun"), "PERSEDIAAN", 0, (ttljual + ongkir - uangmuka));
                    if (sukses) sukses = jur.simpanJurnalSMC(NoNota.getText(),"U","PIUTANG DI "+nmgudang.getText().toUpperCase()+", OLEH "+akses.getkode());
                }
                if(sukses==true){
                    Sequel.Commit();
                }else{
