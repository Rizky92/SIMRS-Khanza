         ...
         try {
             ...
             if(rs.next()){
                ...
                if(sukses==true){
                    ...
                    Sequel.insertTampJurnal(Sequel.cariIsi("select Retur_Ke_Suplayer from set_akun"), "RETUR PEMBELIAN", Sequel.cariDoubleSmc("select sum(total) from detreturbeli where no_retur_beli = ?", rs.getString("no_retur_beli")), 0);
                    Sequel.insertTampJurnal(Sequel.cariIsi("select Kontra_Retur_Ke_Suplayer from set_akun"), "KAS DI TANGAN", 0, Sequel.cariDoubleSmc("select sum(total) from detreturbeli where no_retur_beli = ?", rs.getString("no_retur_beli")));
