           ...
           try {
               ...
               try {
                  ...
                  if(rs.next()){
                      ...
                      if(sukses==true){
                        jur.clear();
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Hibah_Obat from set_akun"), "PERSEDIAAN HIBAH OBAT & BHP", 0, rs.getDouble("totalnilai"));
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Hibah_Obat from set_akun"), "PENDAPATAN HIBAH", rs.getDouble("totalnilai"), 0);
                          if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_hibah"),"U","BATAL HIBAH OBAT & BHP DI "+Sequel.cariIsi("select bangsal.nm_bangsal from bangsal where bangsal.kd_bangsal=?",rs.getString("kd_bangsal")).toUpperCase()+", OLEH "+akses.getkode());
                      }
