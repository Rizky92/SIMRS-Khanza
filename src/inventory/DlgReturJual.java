        ...
        if(NoRetur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses==true){
                    if(!formvalid.equals("No")){
                        jur.clear();
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Retur_Dari_pembeli from set_akun"), "RETUR PENJUALAN", 0, ttlretur);
                        if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Retur_Dari_Pembeli from set_akun"), "KAS DI TANGAN", ttlretur, 0);
                        if (sukses) sukses = jur.simpanJurnalSMC(NoRetur.getText(),"U","RETUR PENJUALAN DI "+nmgudang.getText().toUpperCase()+", OLEH "+akses.getkode());
                    }
                }
                if(sukses==true){
                    Sequel.Commit();
