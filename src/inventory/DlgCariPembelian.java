    ...
    private void ppHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppHapusActionPerformed
        if(tbDokter.getSelectedRow()>-1){
            if(tbDokter.getValueAt(tbDokter.getSelectedRow(),1).toString().trim().equals("")){
                ...
                try {
                    pscaribeli=koneksi.prepareStatement("select pembelian.no_faktur,pembelian.tagihan,pembelian.kd_bangsal,pembelian.tgl_beli,pembelian.kd_rek,pembelian.total2,pembelian.ppn from pembelian where pembelian.no_faktur=?");
                    try {
                        ...
                        if(rs.next()){
                            ...
                            if(sukses==true){
                                jur.clear();
                                if (sukses) sukses = jur.tampung(akunpengadaan, 0, rs.getDouble("total2"));
                                if(rs.getDouble("ppn")>0){
                                }
                                if (sukses) sukses = jur.tampung(rs.getString("kd_rek"), rs.getDouble("tagihan"), 0);
                                if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_faktur"),"U","BATAL PEMBELIAN DI "+Sequel.cariIsi("select bangsal.nm_bangsal from bangsal where bangsal.kd_bangsal=?",rs.getString("kd_bangsal")).toUpperCase()+", OLEH "+akses.getkode());
                            }
                            if(sukses==true){
