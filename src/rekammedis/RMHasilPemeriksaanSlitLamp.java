    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataHasilPemeriksaanSlitLamp.html", "HASIL PEMERIKSAAN SLIT LAMP", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataHasilPemeriksaanSlitLamp.wps", "HASIL PEMERIKSAAN SLIT LAMP", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataHasilPemeriksaanSlitLamp.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataHasilPemeriksaanSlitLamp.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
