    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataHasilEndoskopiHidung.html", "DATA HASIL PEMERIKSAAN TELE ENDOSKOPI HIDUNG", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataHasilEndoskopiHidung.wps", "DATA HASIL PEMERIKSAAN TELE ENDOSKOPI HIDUNG", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataHasilEndoskopiHidung.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataHasilEndoskopiHidung.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
