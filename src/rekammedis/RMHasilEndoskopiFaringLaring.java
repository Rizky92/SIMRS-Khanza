    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataHasilEndoskopiFaringLaring.html", "DATA HASIL PEMERIKSAAN TELE ENDOSKOPI FARING/TELE LARINGOSKOPI", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataHasilEndoskopiFaringLaring.wps", "DATA HASIL PEMERIKSAAN TELE ENDOSKOPI FARING/TELE LARINGOSKOPI", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataHasilEndoskopiFaringLaring.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataHasilEndoskopiFaringLaring.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
    ...
    private void btnDicomActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDicomActionPerformed
        if(tabModeDicom.getRowCount()==0){
            ...
            if(tbListDicom.getSelectedRow()!= -1){
                ...
                try {
                    orthan.loadViewerSmc(tbListDicom.getValueAt(tbListDicom.getSelectedRow(),1).toString(),tbListDicom.getValueAt(tbListDicom.getSelectedRow(),2).toString());
                } catch (Exception ex) {
