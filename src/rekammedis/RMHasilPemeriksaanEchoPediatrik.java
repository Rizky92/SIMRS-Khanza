    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("DataHasilPemeriksaanECHOPediatrik.html", "DATA HASIL PEMERIKSAAN ECHO PEDIATRIK", tbObat);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("DataHasilPemeriksaanECHOPediatrik.wps", "DATA HASIL PEMERIKSAAN ECHO PEDIATRIK", tbObat);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("DataHasilPemeriksaanECHOPediatrik.csv", tbObat);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("DataHasilPemeriksaanECHOPediatrik.xlsx", tbObat);
                        break;
                }
            } catch (Exception e) {
