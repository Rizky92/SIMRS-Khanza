        ...
        if(NoPiutang.getText().trim().equals("")){
            ...
            if(Sequel.cariInteger("select count(piutang_jasa_perusahaan.no_piutang) from piutang_jasa_perusahaan where piutang_jasa_perusahaan.no_piutang=? and piutang_jasa_perusahaan.kode_perusahaan=?",NoPiutang.getText(),KdPerusahaan.getText())>0){
                ...
                if(Sequel.menyimpantf("bayar_piutang_jasa_perusahaan","?,?,?,?,?,?,?","Pembayaran",7,new String[]{
                        ...
                        jur.clear();
                        if (sukses) sukses = jur.tampung(Piutang_Jasa_Perusahaan, "PIUTANG JASA PERUSAHAAN", "0", Cicilan.getText());
                        if (sukses) sukses = jur.tampung(koderekening, AkunBayar.getSelectedItem().toString(), Cicilan.getText(), "0");
                        if (sukses) sukses = jur.simpanJurnalSMC(NoPiutang.getText(),"U","BAYAR PIUTANG JASA PERUSAHAAN"+", OLEH "+akses.getkode());
                }else{
                    sukses=false;
                }
    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tbKamar.getSelectedRow()>-1){
            ...
            if(Sequel.queryu2tf("delete from bayar_piutang_jasa_perusahaan where tgl_bayar=? and kode_perusahaan=? and no_piutang=? and kd_rek=? and nama_bayar=?", 5,new String[]{
                ...
                jur.clear();
                if (sukses) sukses = jur.tampung(Piutang_Jasa_Perusahaan, "PIUTANG JASA PERUSAHAAN", tbKamar.getValueAt(tbKamar.getSelectedRow(), 3).toString(), "0");
                if (sukses) sukses = jur.tampung(tbKamar.getValueAt(tbKamar.getSelectedRow(), 6).toString(), tbKamar.getValueAt(tbKamar.getSelectedRow(), 7).toString(), "0", tbKamar.getValueAt(tbKamar.getSelectedRow(), 3).toString());
                if (sukses) sukses = jur.simpanJurnalSMC(NoPiutang.getText(),"U","PEMBATALAN BAYAR PIUTANG JASA PERUSAHAAN"+", OLEH "+akses.getkode());
            }else{
                sukses=false;
            }
