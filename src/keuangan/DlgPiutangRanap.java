    ...
    public DlgPiutangRanap(java.awt.Frame parent, boolean modal) {
        ...
        tabMode=new DefaultTableModel(null,rowRwJlDr){
            ...
            public Class getColumnClass(int columnIndex) {
                if (columnIndex >= 7 && columnIndex <= 24) {
                    return Double.class;
                }
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        ...
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("PiutangRanap.wps", "LAPORAN PIUTANG RANAP PER TGL. PULANG " + Tgl1.getSelectedItem().toString() + " s.d. " + Tgl2.getSelectedItem().toString(), tbBangsal);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("PiutangRanap.csv", tbBangsal);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("PiutangRanap.xlsx", tbBangsal);
                        break;
            ...
            new SwingWorker<Void, Object[]>() {
                        ...
                        try (ResultSet rs = ps.executeQuery()) {
                            while (rs.next()) {
                                ...
                                publish(new Object[] {
                                    ...
                                    rs.getString("png_jawab") + " (" + rs.getString("kd_pj") + ")", rs.getString("asal_rujuk"), rs.getDouble("registrasi"), rs.getDouble("ranap_dr") +
                                    rs.getDouble("ranap_drpr") + rs.getDouble("ranap_pr") + rs.getDouble("ralan_dr") + rs.getDouble("ralan_drpr") + rs.getDouble("ralan_pr"),
                                    rs.getDouble("obat"), rs.getDouble("retur_obat"), rs.getDouble("resep_pulang"), rs.getDouble("laborat"), rs.getDouble("radiologi"), rs.getDouble("potongan"),
                                    rs.getDouble("tambahan"), rs.getDouble("kamar") + rs.getDouble("service"), rs.getDouble("operasi"), rs.getDouble("harian"), rs.getDouble("laborat") +
                                    rs.getDouble("radiologi") + rs.getDouble("operasi") + rs.getDouble("obat") + rs.getDouble("ranap_dr") + rs.getDouble("ranap_drpr") + rs.getDouble("ranap_pr") +
                                    rs.getDouble("ralan_dr") + rs.getDouble("ralan_drpr") + rs.getDouble("ralan_pr") + rs.getDouble("tambahan") + rs.getDouble("potongan") + rs.getDouble("kamar") +
                                    rs.getDouble("registrasi") + rs.getDouble("harian") + rs.getDouble("retur_obat") + rs.getDouble("resep_pulang") + rs.getDouble("service"), rs.getDouble("uangmuka"),
                                    rs.getDouble("besar_cicilan"), rs.getDouble("diskon_piutang"), rs.getDouble("tidak_terbayar"), rs.getDouble("totalpiutang") - rs.getDouble("uangmuka") -
                                    rs.getDouble("besar_cicilan") - rs.getDouble("diskon_piutang") - rs.getDouble("tidak_terbayar"), rs.getString("kd_kamar") + " " + rs.getString("nm_bangsal")
                                });
                            }
                        }
                    } catch (Exception e) {
                ...
                protected void done() {
                    tabMode.addRow(new Object[] {
                        ">> Total", ":", "", "", "", "", "", ttlRegistrasi, ttlRanap_Dokter + ttlRanap_Paramedis + ttlRalan_Dokter + ttlRalan_Paramedis, ttlObat, ttlRetur_Obat, ttlResep_Pulang,
                        ttlLaborat, ttlRadiologi, ttlPotongan, ttlTambahan, ttlKamar + ttlService, ttlOperasi, ttlHarian, all, ttlekses, ttldibayar, ttldiskon, ttltidakdibayar, ttlsisa, ""
                    });
