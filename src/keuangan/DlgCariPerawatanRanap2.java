                ...
                if(pilihtable.equals("rawat_inap_dr")||pilihtable.equals("rawat_inap_pr")||pilihtable.equals("rawat_inap_drpr")){
                    ...
                    if(sukses==true){
                        ...
                        if(hapusttlpendapatan>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Suspen_Piutang_Tindakan_Ranap, "Suspen Piutang Tindakan Ranap", 0, hapusttlpendapatan);
                            if (sukses) sukses = Sequel.insertTampJurnal(Tindakan_Ranap, "Pendapatan Tindakan Ranap", hapusttlpendapatan, 0);
                        }
                        if(hapusttljmdokter>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Dokter_Tindakan_Ranap, "Beban Jasa Medik Dokter Tindakan Ranap", 0, hapusttljmdokter);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Dokter_Tindakan_Ranap, "Utang Jasa Medik Dokter Tindakan Ranap", hapusttljmdokter, 0);
                        }
                        if(hapusttljmperawat>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Paramedis_Tindakan_Ranap, "Beban Jasa Medik Paramedis Tindakan Ranap", 0, hapusttljmperawat);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Paramedis_Tindakan_Ranap, "Utang Jasa Medik Paramedis Tindakan Ranap", hapusttljmperawat, 0);
                        }
                        if(hapusttlkso>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_KSO_Tindakan_Ranap, "Beban KSO Tindakan Ranap", 0, hapusttlkso);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_KSO_Tindakan_Ranap, "Utang KSO Tindakan Ranap", hapusttlkso, 0);
                        }
                        if(hapusttlmenejemen>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Menejemen_Tindakan_Ranap, "Beban Jasa Menejemen Tindakan Ranap", 0, hapusttlmenejemen);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Menejemen_Tindakan_Ranap, "Utang Jasa Menejemen Tindakan Ranap", hapusttlmenejemen, 0);
                        }
                        if(hapusttljasasarana>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Sarana_Tindakan_Ranap, "Beban Jasa Sarana Tindakan Ranap", 0, hapusttljasasarana);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Sarana_Tindakan_Ranap, "Utang Jasa Sarana Tindakan Ranap", hapusttljasasarana, 0);
                        }
                        if(hapusttlbhp>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(HPP_BHP_Tindakan_Ranap, "HPP BHP Tindakan Ranap", 0, hapusttlbhp);
                            if (sukses) sukses = Sequel.insertTampJurnal(Persediaan_BHP_Tindakan_Ranap, "Persediaan BHP Tindakan Ranap", hapusttlbhp, 0);
                        }
                        if (sukses) sukses = jur.simpanJurnal(TNoRw.getText(),"U","PEMBATALAN TINDAKAN RAWAT INAP PASIEN OLEH "+akses.getkode());  
                    }
                                                                      
                    if(sukses==true){
                        Sequel.deleteTampJurnal();
                        if(ttlpendapatan>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Suspen_Piutang_Tindakan_Ranap, "Suspen Piutang Tindakan Ranap", ttlpendapatan, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Tindakan_Ranap, "Pendapatan Tindakan Ranap", 0, ttlpendapatan);
                        }
                        if(ttljmdokter>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Dokter_Tindakan_Ranap, "Beban Jasa Medik Dokter Tindakan Ranap", ttljmdokter, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Dokter_Tindakan_Ranap, "Utang Jasa Medik Dokter Tindakan Ranap", 0, ttljmdokter);
                        }
                        if(ttljmperawat>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Medik_Paramedis_Tindakan_Ranap, "Beban Jasa Medik Paramedis Tindakan Ranap", ttljmperawat, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Medik_Paramedis_Tindakan_Ranap, "Utang Jasa Medik Paramedis Tindakan Ranap", 0, ttljmperawat);
                        }
                        if(ttlkso>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_KSO_Tindakan_Ranap, "Beban KSO Tindakan Ranap", ttlkso, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_KSO_Tindakan_Ranap, "Utang KSO Tindakan Ranap", 0, ttlkso);
                        }
                        if(ttljasasarana>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Sarana_Tindakan_Ranap, "Beban Jasa Sarana Tindakan Ranap", ttljasasarana, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Sarana_Tindakan_Ranap, "Utang Jasa Sarana Tindakan Ranap", 0, ttljasasarana);
                        }
                        if(ttlbhp>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(HPP_BHP_Tindakan_Ranap, "HPP BHP Tindakan Ranap", ttlbhp, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Persediaan_BHP_Tindakan_Ranap, "Persediaan BHP Tindakan Ranap", 0, ttlbhp);
                        }
                        if(ttlmenejemen>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Beban_Jasa_Menejemen_Tindakan_Ranap, "Beban Jasa Menejemen Tindakan Ranap", ttlmenejemen, 0);
                            if (sukses) sukses = Sequel.insertTampJurnal(Utang_Jasa_Menejemen_Tindakan_Ranap, "Utang Jasa Menejemen Tindakan Ranap", 0, ttlmenejemen);
                        }
