    ...
    try {
        ...
        if(i>0){
            if(!tbBilling.getValueAt(0,1).toString().trim().contains("No.Nota")){
                ...
                if((-1*ttlPotongan)>0){
                    if (sukses) sukses = Sequel.insertTampJurnal(Potongan_Ralan, "Potongan Ralan", 0, (-1 * ttlPotongan));
                }

                if((ttlRalan_Dokter+ttlRalan_Dokter_Param+ttlRalan_Paramedis)>0){
                    if (sukses) sukses = Sequel.insertTampJurnal(Tindakan_Ralan, "Tindakan Ralan", (ttlRalan_Dokter + ttlRalan_Dokter_Param + ttlRalan_Paramedis), 0);
                }
                ...
                if((ttlObat-obatlangsung-ppnobat)>0){
                    if (sukses) sukses = Sequel.insertTampJurnal(Obat_Ralan, "Obat Ralan", (ttlObat - obatlangsung - ppnobat), 0);
                }

                if(obatlangsung>0){
                    if (sukses) sukses = Sequel.insertTampJurnal(Obat_Langsung_Ralan, "Obat Langsung Ralan", obatlangsung, 0);
                }
                ...
                if(ttlTambahan>0){
                    if (sukses) sukses = Sequel.insertTampJurnal(Tambahan_Ralan, "Tambahan Ralan", ttlTambahan, 0);
                }

                if(ttlOperasi>0){
                    if (sukses) sukses = Sequel.insertTampJurnal(Operasi_Ralan, "Operasi Ralan", ttlOperasi, 0);
                }
        ...
        if((chkLaborat.isSelected()==false)||(chkRadiologi.isSelected()==false)){
            ...
            try {
                ...
                if(sukses==true){
                    ...
                    if(sukses==true){
                        if((-1 * ttlPotongan) > 0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Potongan_Ralan, "Potongan Ralan", (-1 * ttlPotongan), 0);
                        }

                        if((ttlRalan_Dokter+ttlRalan_Dokter_Param+ttlRalan_Paramedis)>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Tindakan_Ralan, "Tindakan Ralan", 0, (ttlRalan_Dokter + ttlRalan_Dokter_Param + ttlRalan_Paramedis));
                        }
                        ...
                        if((ttlObat-obatlangsung-ppnobat)>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Obat_Ralan, "Obat Ralan", 0, (ttlObat - obatlangsung - ppnobat));
                        }
                        
                        if(obatlangsung>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Obat_Langsung_Ralan, "Obat Langsung Ralan", 0, obatlangsung);
                        }
                        ...
                        if(ttlTambahan>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Tambahan_Ralan, "Tambahan Ralan", 0, ttlTambahan);
                        }

                        if(ttlOperasi>0){
                            if (sukses) sukses = Sequel.insertTampJurnal(Operasi_Ralan, "Operasi Ralan", 0, ttlOperasi);
                        }
