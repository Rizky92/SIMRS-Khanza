                        ...
                        if(rs.next()){
                            ...
                            if(sukses==true){
                                ...
                                if(rs.getDouble("rawatjalan")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Tindakan_Ralan, "Utang Jasa Medik Dokter Tindakan Ralan", 0, rs.getDouble("rawatjalan"));
                                }
                                if(rs.getDouble("rawatinap")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Tindakan_Ranap, "Utang Jasa Medik Dokter Tindakan Ranap", 0, rs.getDouble("rawatinap"));
                                }
                                if(rs.getDouble("labrawatjalan")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Laborat_Ralan, "Utang Jasa Medik Dokter Laborat Ralan", 0, rs.getDouble("labrawatjalan"));
                                }
                                if(rs.getDouble("labrawatinap")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Laborat_Ranap, "Utang Jasa Medik Dokter Laborat Ranap", 0, rs.getDouble("labrawatinap"));
                                }
                                if(rs.getDouble("radrawatjalan")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Radiologi_Ralan, "Utang Jasa Medik Dokter Radiologi Ralan", 0, rs.getDouble("radrawatjalan"));
                                }
                                if(rs.getDouble("radrawatinap")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Radiologi_Ranap, "Utang Jasa Medik Dokter Radiologi Ranap", 0, rs.getDouble("radrawatinap"));
                                }
                                if(rs.getDouble("operasiralan")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Operasi_Ralan, "Utang Jasa Medik Dokter Operasi Ralan", 0, rs.getDouble("operasiralan"));
                                }
                                if(rs.getDouble("operasiranap")>0){
                                    if (sukses) sukses = jur.tampung(Utang_Jasa_Medik_Dokter_Operasi_Ranap, "Utang Jasa Medik Dokter Operasi Ranap", 0, rs.getDouble("operasiranap"));
                                }
                                if(totaltagihan>0){
                                    if (sukses) sukses = jur.tampung(Akun_Biaya_Mandiri, "BIAYA TRANSAKSI", 0, totaltagihan);
                                }
                                if(rs.getDouble("besar_bayar")>0){
                                    if (sukses) sukses = jur.tampung(koderekening, rs.getString("nama_bayar"), totaltagihan + rs.getDouble("besar_bayar"), 0);
                                }
                                if (sukses) sukses = jur.simpanJurnalSMC(tbDokter.getValueAt(tbDokter.getSelectedRow(),0).toString(),"U","PEMBATALAN PEMBAYARAN JASA MEDIS DOKTER "+tbDokter.getValueAt(tbDokter.getSelectedRow(),3).toString()+" "+tbDokter.getValueAt(tbDokter.getSelectedRow(),4).toString()+", OLEH "+akses.getkode());
                            }
