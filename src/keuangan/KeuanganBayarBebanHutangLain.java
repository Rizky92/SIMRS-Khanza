    ...
    private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
        if(tbKamar.getSelectedRow()>-1){
            ...
            if(Sequel.queryu2tf("delete from bayar_beban_hutang_lain where tgl_bayar=? and kode_pemberi_hutang=? and no_hutang=? and kd_rek=? and nama_bayar=?", 5,new String[]{
                ...
                jur.bersihkan();
                if (sukses) sukses = jur.tampung(kontraakun, namakontraakun, "0", tbKamar.getValueAt(tbKamar.getSelectedRow(), 3).toString());
                if (sukses) sukses = jur.tampung(tbKamar.getValueAt(tbKamar.getSelectedRow(), 6).toString(), tbKamar.getValueAt(tbKamar.getSelectedRow(), 7).toString(), tbKamar.getValueAt(tbKamar.getSelectedRow(), 3).toString(), "0");
