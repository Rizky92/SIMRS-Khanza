    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(Nama.getText().trim().equals("")||Nip.getText().trim().equals("")){
            ...
            if (i == JOptionPane.YES_OPTION) {
                ...
                if(sukses==true){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Pengambilan_Penunjang_Utd from set_akun"), "PENGAMBILAN BARANG NON MEDIS UTD", subtotal, 0);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Pengambilan_Penunjang_Utd from set_akun"), "PERSEDIAAN BARANG NON MEDIS", 0, subtotal);
                    if (sukses) sukses = jur.simpanJurnalSMC(Valid.SetTgl(Tanggal.getSelectedItem()+"").replaceAll("-","/"),"U","PENGAMBILAN BARANG NON MEDIS UTD"+", OLEH "+akses.getkode());
                }
