     ...
     try {
         ...
         try {
            ...
            if(rs.next()){
                ...
                jur.clear();
                if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Penerimaan_NonMedis from set_akun"), "PERSEDIAAN BARANG NON MEDIS", 0, rs.getDouble("total"));
                if (rs.getDouble("ppn") > 0) {
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select PPN_Masukan from set_akun"), "PPN Masukan Barang Non Medis", 0, rs.getDouble("ppn"));
                }
                if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Penerimaan_NonMedis from set_akun"), "HUTANG BARANG NON MEDIS", rs.getDouble("tagihan"), 0);
                if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_faktur"),"U","BATAL TRANSAKSI PENERIMAAN BARANG NON MEDIS"+", OLEH "+akses.getkode());
