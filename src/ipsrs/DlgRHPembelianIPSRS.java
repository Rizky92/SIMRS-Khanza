    ...
    private void BtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnPrintActionPerformed
        ...
        if(tabMode.getRowCount()==0){
            ...
            try {
                ...
                switch (pilihan) {
                    case "Laporan 1 (HTML)":
                        Valid.exportHtmlSmc("RekapPengadaanNonMedis.html", "Rekap Pengadaan Barang Non Medis", tbDokter);
                        break;
                    case "Laporan 2 (WPS)":
                        Valid.exportWPSSmc("RekapPengadaanNonMedis.wps", "Rekap Pengadaan Barang Non Medis", tbDokter);
                        break;
                    case "Laporan 3 (CSV)":
                        Valid.exportCSVSmc("RekapPengadaanNonMedis.csv", tbDokter);
                        break;
                    case "Laporan 4 (XLSX)":
                        Valid.exportXlsxSmc("RekapPengadaanNonMedis.xlsx", tbDokter);
                        break;
                    case "Laporan 5 (Jasper)":
                        ...
                        Valid.reportSmc("rptRekapPengadaanNonMedis.jasper", "report", "::[ Rekap Pengadaan Barang Non Medis ]::", param, "select ipsrspembelian.tgl_beli, ipsrspembelian.no_faktur, " +
                            "ipsrspembelian.kode_suplier, ipsrssuplier.nama_suplier, ipsrspembelian.nip, petugas.nama, ipsrsdetailbeli.kode_brng, ipsrsbarang.nama_brng, ipsrsdetailbeli.kode_sat, " +
                            "kodesatuan.satuan, ipsrsdetailbeli.jumlah, ipsrsdetailbeli.harga, ipsrsdetailbeli.subtotal, ipsrsdetailbeli.dis, ipsrsdetailbeli.besardis, ipsrsdetailbeli.total from " +
                            "ipsrspembelian inner join ipsrssuplier on ipsrspembelian.kode_suplier = ipsrssuplier.kode_suplier inner join petugas on ipsrspembelian.nip = petugas.nip inner join " +
                            "ipsrsdetailbeli on ipsrspembelian.no_faktur = ipsrsdetailbeli.no_faktur inner join ipsrsbarang on ipsrsdetailbeli.kode_brng = ipsrsbarang.kode_brng inner join " +
                            "kodesatuan on ipsrsdetailbeli.kode_sat = kodesatuan.kode_sat where ipsrspembelian.tgl_beli between ? and ? and (if(trim(?) = '', 1 = 1, ipsrspembelian.no_faktur = ?)) " +
                            "and (if(trim(?) = '', 1 = 1, ipsrspembelian.kode_suplier = ?)) and (if(trim(?) = '', 1 = 1, ipsrspembelian.nip = ?)) and (if(trim(?) = '', 1 = 1, ipsrsbarang.jenis = ?)) " +
                            "and (if(trim(?) = '', 1 = 1, ipsrsdetailbeli.kode_brng = ?)) and (if(trim(?) = '', 1 = 1, ipsrspembelian.no_faktur like ? or ipsrspembelian.kode_suplier like ? or " +
                            "ipsrssuplier.nama_suplier like ? or ipsrspembelian.nip like ? or petugas.nama like ? or ipsrsdetailbeli.kode_brng like ? or ipsrsbarang.nama_brng like ? or " +
                            "ipsrsdetailbeli.kode_sat like ?)) order by ipsrspembelian.tgl_beli, ipsrspembelian.no_faktur", Valid.getTglSmc(TglBeli1), Valid.getTglSmc(TglBeli2), NoFaktur.getText().trim(),
                            NoFaktur.getText(), kdsup.getText().trim(), kdsup.getText(), kdptg.getText().trim(), kdptg.getText(), kdjenis.getText().trim(), kdjenis.getText(), kdbar.getText().trim(),
                            kdbar.getText(), TCari.getText().trim(), "%" + TCari.getText().trim() + "%", "%" + TCari.getText().trim() + "%", "%" + TCari.getText().trim() + "%", "%" + 
