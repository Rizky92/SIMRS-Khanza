    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if(NoFaktur.getText().trim().equals("")){
            ...
            if (reply == JOptionPane.YES_OPTION) {
                ...
                if(sukses==true){
                    jur.clear();
                    if (sukses) sukses = jur.tampung(akunpembelian, "PEMBELIAN", (ttl + meterai), 0);
                    if (ppn > 0) {
                        if (sukses) sukses = jur.tampung(PPN_Masukan, "PPN Masukan IPSRS", ppn, 0);
                    }
                    if (sukses) sukses = jur.tampung(akunbayar, "KAS KELUAR", 0, (ttl + ppn + meterai));
                    if (sukses) sukses = jur.simpanJurnalSMC(NoFaktur.getText(),"U","PEMBELIAN BARANG NON MEDIS DAN PENUNJANG(LAB & RAD) "+", OLEH "+akses.getkode());
                }
                if(sukses==true){
                    Sequel.Commit();
                    jml=tbDokter.getRowCount();
