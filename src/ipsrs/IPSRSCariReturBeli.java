          ...
          try {
             ...
             try {
                ...
                if(rs.next()){
                    ...
                    jur.clear();
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Retur_Beli_Non_Medis from set_akun"), "RETUR BELI NON MEDIS", rs.getDouble("total"), 0);
                    if (sukses) sukses = jur.tampung(Sequel.cariIsi("select Kontra_Retur_Beli_Non_Medis from set_akun"), "KONTA RETUR BELI NON MEDIS", 0, rs.getDouble("total"));
                    if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_retur_beli"),"U","BATAL TRANSAKSI RETUR BELI BARANG PENUNJANG/NON MEDIS"+", OLEH "+akses.getkode());
