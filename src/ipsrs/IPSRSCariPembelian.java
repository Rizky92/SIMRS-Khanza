               ...
               try {
                  ...
                  if(rs.next()){
                      ...
                      if (rs.getDouble("ppn") > 0) {
                        if (sukses) sukses = jur.tampung(PPN_Masukan, "PPN Masukan IPSRS", 0, rs.getDouble("ppn"));
                      }

                      if (sukses) sukses = jur.tampung(rs.getString("kd_rek"), "KAS DI TANGAN", rs.getDouble("tagihan"), 0);

                      if (sukses) sukses = jur.simpanJurnalSMC(rs.getString("no_faktur"),"U","PEMBATALAN PEMBELIAN BARANG NON MEDIS DAN PENUNJANG (LAB & RAD)"+", OLEH "+akses.getkode());
                      if(sukses==true){
