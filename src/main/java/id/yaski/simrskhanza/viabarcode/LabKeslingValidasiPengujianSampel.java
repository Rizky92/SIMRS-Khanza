...
public final class LabKeslingValidasiPengujianSampel extends javax.swing.JDialog {
    ...
    private void initComponents() {
        ...
        Rentang = new id.yaski.simrskhanza.widget.TextBox();
        NoVerifikasi = new id.yaski.simrskhanza.widget.TextBox();
        ...
        TanggalValidasi = new id.yaski.simrskhanza.widget.Tanggal();
        ...
        NmPJ = new id.yaski.simrskhanza.widget.TextBox();
        btnPJ = new id.yaski.simrskhanza.widget.Button();
        jLabel15 = new id.yaski.simrskhanza.widget.Label();
        jLabel4 = new id.yaski.simrskhanza.widget.Label();
        TNoPermintaan = new id.yaski.simrskhanza.widget.TextBox();
        ...
        TNoValidasi = new id.yaski.simrskhanza.widget.TextBox();
        ...
        KodeVerifikator = new id.yaski.simrskhanza.widget.TextBox();
        NamaVerifikator = new id.yaski.simrskhanza.widget.TextBox();
                ...
                try {
                    ...
                    if(Sequel.menyimpantf2("labkesling_validasi_pengujian_sampel","?,?,?,?,?,?,'Belum Bayar'","No.Validasi",6,new String[]{
                        ...
                        if(berhasil==true){
                            ...
                            if(jasa_sarana>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_Jasa_Sarana_Pelayanan_Lab_Kesling, "Beban Jasa Sarana Pelayanan Lab Kesling", jasa_sarana, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_Jasa_sarana_Pelayanan_Lab_Kesling, "Utang Jasa Sarana Pelayanan Lab Kesling", 0, jasa_sarana);
                            }
                            if(paket_bhp>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(HPP_BHP_Pelayanan_Lab_Kesling, "HPP BHP Pelayanan Lab Kesling", paket_bhp, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Persediaan_BHP_Pelayanan_Lab_Kesling, "Persediaan BHP Pelayanan Lab Kesling", 0, paket_bhp);
                            }
                            if(jasa_pj_lab>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_Jasa_PJLab_Pelayanan_Lab_Kesling, "Beban Jasa PJ Lab Pelayanan Lab Kesling", jasa_pj_lab, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_Jasa_PJLab_Pelayanan_Lab_Kesling, "Utang Jasa PJ Lab Pelayanan Lab Kesling", 0, jasa_pj_lab);
                            }
                            if(jasa_pj_pengujian>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_Jasa_PJPengujian_Pelayanan_Lab_Kesling, "Beban Jasa PJ Pengujian Pelayanan Lab Kesling", jasa_pj_pengujian, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_Jasa_PJPengujian_Pelayanan_Lab_Kesling, "Utang Jasa PJ Pengujian Pelayanan Lab Kesling", 0, jasa_pj_pengujian);
                            }
                            if(jasa_verifikator>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_Jasa_PJVerifikasi_Pelayanan_Lab_Kesling, "Beban Jasa PJ Verifikasi Pelayanan Lab Kesling", jasa_verifikator, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_Jasa_PJVerifikasi_Pelayanan_Lab_Kesling, "Utang Jasa PJ Verifikasi Pelayanan Lab Kesling", 0, jasa_verifikator);
                            }
                            if(jasa_petugas>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_Jasa_Analis_Pelayanan_Lab_Kesling, "Beban Jasa Analis Pelayanan Lab Kesling", jasa_petugas, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_Jasa_Analis_Pelayanan_Lab_Kesling, "Utang Jasa Analis Pelayanan Lab Kesling", 0, jasa_petugas);
                            }
                            if(kso>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_KSO_Pelayanan_Lab_Kesling, "Beban KSO Pelayanan Lab Kesling", kso, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_KSO_Pelayanan_Lab_Kesling, "Utang KSO Pelayanan Lab Kesling", 0, kso);
                            }
                            if(jasa_menejemen>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Beban_Jasa_Menejemen_Pelayanan_Lab_Kesling, "Beban Jasa Menejemen Pelayanan Lab Kesling", jasa_menejemen, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Utang_Jasa_Menejemen_Pelayanan_Lab_Kesling, "Utang Jasa Menejemen Pelayanan Lab Kesling", 0, jasa_menejemen);
                            }
                            if(total>0){
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Suspen_Piutang_Pelayanan_Lab_Kesling, "Suspen Piutang Pelayanan Lab Kesling", total, 0);
                                if (berhasil) berhasil = Sequel.insertOrUpdateTampJurnal(Pendapatan_Pelayanan_Lab_Kesling, "Pendapatan Pelayanan Lab Kesling", 0, total);
                            }
                            if (berhasil) berhasil = jur.simpanJurnal(TNoValidasi.getText(), "U", "PELAYANAN LABORATORIUM KESEHATAN LINGKUNGAN " + NamaPelanggan.getText() + " DIPOSTING OLEH " + akses.getkode());
                        }
                    }else{
                        berhasil=false;
                    }
    ...
    */
    ...
    private id.yaski.simrskhanza.widget.TextBox KodeVerifikator;
    ...
    private id.yaski.simrskhanza.widget.TextBox NamaVerifikator;
    private id.yaski.simrskhanza.widget.TextBox NmPJ;
    private id.yaski.simrskhanza.widget.TextBox NoVerifikasi;
    private id.yaski.simrskhanza.widget.PanelBiasa PanelInput;
    private id.yaski.simrskhanza.widget.TextBox Rentang;
    private id.yaski.simrskhanza.widget.ScrollPane Scroll2;
    private id.yaski.simrskhanza.widget.TextBox TNoPermintaan;
    private id.yaski.simrskhanza.widget.TextBox TNoValidasi;
    private id.yaski.simrskhanza.widget.Tanggal TanggalValidasi;
    private id.yaski.simrskhanza.widget.Button btnPJ;
