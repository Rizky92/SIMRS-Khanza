...
ALTER TABLE `surat_keterangan_rawat_inap` ADD COLUMN IF NOT EXISTS `lamasakit` varchar(20) NULL DEFAULT NULL AFTER `tanggalakhir`;

ALTER TABLE `surat_keterangan_rawat_inap` ADD COLUMN IF NOT EXISTS `kd_dokter` varchar(20) NOT NULL AFTER `lamasakit`;

ALTER TABLE `surat_keterangan_rawat_inap` ADD CONSTRAINT FOREIGN KEY IF NOT EXISTS `surat_keterangan_rawat_inap_dokter_FK` (`kd_dokter`) REFERENCES `dokter` (`kd_dokter`) ON DELETE RESTRICT ON UPDATE CASCADE;
