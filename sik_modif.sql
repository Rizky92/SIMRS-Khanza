...
ALTER TABLE `inventaris` ADD COLUMN IF NOT EXISTS `no_faktur` varchar(20) NULL DEFAULT NULL AFTER `no_box`;
...
CREATE TABLE IF NOT EXISTS `surat_pemesanan_inventaris_smc` (
  ...
  CONSTRAINT `surat_pemesanan_inventaris_smc_ibfk_3` FOREIGN KEY (`kd_rek`) REFERENCES `rekening` (`kd_rek`) ON DELETE RESTRICT ON UPDATE CASCADE
) ENGINE = InnoDB DEFAULT CHARSET = latin1 COLLATE = latin1_swedish_ci ROW_FORMAT = Dynamic;
...
ALTER TABLE `user` ADD COLUMN IF NOT EXISTS `ringkasan_barang_belum_inventaris_smc` enum('true','false') DEFAULT NULL AFTER `surat_penolakan_resusitasi`;
