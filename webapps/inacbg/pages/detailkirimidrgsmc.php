...
<div id="post">
    <?php if ((isset($_GET['action']) ? validTeks($_GET['action']) : null) === 'selesai'): ?>
        ...
        <div class="entry" style="font-family: Tahoma; margin-top: 0.5rem; margin-left: 0.5rem">
            <a href="?act=DetailKirimIDRGSmc&<?= $queryurl ?>">[Kirim Ulang]</a>
            <br />
            <br />
            <a href="?act=DetailKirimIDRGSmc&<?= $queryurl ?>&action=cetak">[Tarik ulang hasil cetak klaim]</a>
        </div>
    <?php elseif ((isset($_GET['action']) ? validTeks($_GET['action']) : null) === 'cetak'): ?>
        ...
        <?php if (CetakKlaimSmc($nosep)['success']): ?>
            <meta http-equiv="refresh" content="2;URL=?act=DetailKirimIDRGSmc&<?= $queryurl ?>">
        <?php endif; ?>
    <?php else: ?>
                        ...
                        if ((int) round($validasi) === 0) {
                            if ((!empty($norawat)) && (!empty($nosep)) && (!empty($nokartu))) {
                                ...
                                if (! $success) {
                                    echo $error;
                                    echo <<<HTML
                                        <meta http-equiv="refresh" content="2;URL=?act=DetailKirimIDRGSmc&codernik={$codernik}&nosep={$nosep}&carabayar={$carabayar}&corona={$corona}&sukses=false">
                                        HTML;
                                } else if ($success && $response == 'stage2') {
                                    echo <<<HTML
                                        <meta http-equiv="refresh" content="2;URL=?act=DetailKirimIDRGSmc&codernik={$codernik}&nosep={$nosep}&carabayar={$carabayar}&corona={$corona}&sukses=true&action=stage2">
                                        HTML;
                                } else {
                                    echo <<<HTML
                                        <meta http-equiv="refresh" content="2;URL=?act=DetailKirimIDRGSmc&codernik={$codernik}&nosep={$nosep}&carabayar={$carabayar}&corona={$corona}&sukses=true&action=selesai">
                                        HTML;
                                }
                            } else {
